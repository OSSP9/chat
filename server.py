...
class Worker:
    ...
    def who(self, cid, gname=None):
        ...
        if gname not in self.groups:
            ...
            return
        self.controller.send(cid,
                             DELIM.join((b'users', FIELDSEP.join(self.clientnames[n] for n in self.groups[gname]))))
...
def checkname(name):
    return len(name) <= MAXNAMELEN and bool(
        re.match(b'[ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890_]+', name))
